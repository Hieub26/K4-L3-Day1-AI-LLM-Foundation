...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for res in results:
        p = res["prompt"]
        g4o = (res["gpt4o_response"][:40] + "..." if len(res["gpt4o_response"]) > 40 else res["gpt4o_response"])
        mini = (res["mini_response"][:40] + "..." if len(res["mini_response"]) > 40 else res["mini_response"])
        lines.append(f"{p} | {g4o} | {mini} | {res['gpt4o_latency']:.2f}s | {res['mini_latency']:.2f}s")
    return "\n".join(lines)
